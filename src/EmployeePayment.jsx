...
export default function EmployeePayment({ employeeId, employeeName, onBack }) {
    ...
    const employeeSummary = useMemo(() => {
        ...
        const basicSalary = Number(selectedPayslip.basic_salary || 0);
        const leaveDeduction = Number(selectedPayslip.leave_deduction || 0);
        const equipmentRecovery = Number(selectedPayslip.equipment_recovery || 0);
        const loanDeduction = Number(selectedPayslip.loan_deduction || 0);
        const totalAdvancesDeducted = Number(selectedPayslip.total_advance || 0);

        const totalDeductions = leaveDeduction + equipmentRecovery + loanDeduction + totalAdvancesDeducted;
        ...
        return {
            ...
            advances: selectedPayslip.advances,
                            ...
                            <div className="max-w-4xl mx-auto space-y-4">
                                ...
                                <div className="bg-white rounded-xl shadow-md p-6">
                                    ...
                                    <div className="space-y-4">
                                        {employeeSummary.leaveDeduction > 0 && (
                                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                                <div>
                                                ...
                                                <div>
                                                    <label className="block text-sm font-medium text-neutral-700 mb-1">Reason (Leave)</label>
                                                    <p className="w-full px-4 py-2 border rounded-lg bg-neutral-100 border-neutral-200 text-neutral-800">
                                                        {selectedPayslip.leave_deduction_message || "N/A"}
                                                    </p>
                                                </div>
                                            </div>
                                        )}
                                        {employeeSummary.equipmentRecovery > 0 && (
                                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                                <div>
                                                ...
                                                <div>
                                                    <label className="block text-sm font-medium text-neutral-700 mb-1">Reason (Equipment)</label>
                                                    <p className="w-full px-4 py-2 border rounded-lg bg-neutral-100 border-neutral-200 text-neutral-800">
                                                        {selectedPayslip.equipment_recovery_message || "N/A"}
                                                    </p>
                                                </div>
                                            </div>
                                        )}
                                        {employeeSummary.loanDeduction > 0 && (
                                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                                <div>
                                                ...
                                                <div>
                                                    <label className="block text-sm font-medium text-neutral-700 mb-1">Reason (Loan)</label>
                                                    <p className="w-full px-4 py-2 border rounded-lg bg-neutral-100 border-neutral-200 text-neutral-800">
                                                        {selectedPayslip.loan_deduction_message || "N/A"}
                                                    </p>
                                                </div>
                                            </div>
                                        )}
                                ...
                                {/* Advances Section */}
                                <div className="bg-white rounded-xl shadow-md p-6">
                                    <h3 className="text-lg font-semibold text-neutral-800 mb-4">Advances</h3>
                                    {employeeSummary.advances && employeeSummary.advances.length > 0 ? (
                                        <div className="space-y-4">
                                            {employeeSummary.advances.map((advance, index) => (
                                                <div key={index} className="grid grid-cols-1 md:grid-cols-3 gap-4 bg-neutral-50 p-3 rounded-lg">
                                                    <div>
                                        ...
                                        <p className="text-neutral-500 text-sm">No advances recorded for this payslip.</p>
                                    )}
                                </div>
                            </div>
                        </main>
